...
from data.pseudo_dataset import PseudoDatasetForEvaluation
...
class ImageSofaset(torch.utils.data.Dataset):
    # python run_generation.py --name sofa_hd  --dataset sofa --gpu_ids 0 --batch_size 3 --epochs 600
    ...
    def __getitem__(self,index):
        ...
        data = np.load(os.path.join(self.cache_dir,
                       f'pseudogt_{self.res}x{self.res}',
                       f'{index}.npz'), allow_pickle=True)
...
if args.dataset == 'p3d':
    from data.p3d_pseudo_dataset import P3dPseudoDataset
    train_ds = P3dPseudoDataset(args)
elif args.dataset == 'cub':
    from data.cub_pseudo_dataset import CubPseudoDataset
    train_ds = CubPseudoDataset(args)
elif args.dataset == 'sofa':
    train_ds = ImageSofaset(args)
else:
